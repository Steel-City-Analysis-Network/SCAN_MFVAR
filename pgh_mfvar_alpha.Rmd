...
```{r} 
mod.minn.diffuse <- estimate_mfbvar(prior.exo, prior = "minn", vairnace = "diffuse") 
...
df.mod.minn.diffuse95 <- predict(mod.minn.diffuse, aggregate_fcst = TRUE, pred_bands = .95)
...
```{r} 
df.mod.pgh.minn.iw <- predict(mod.pgh.minn.iw, aggregate_fcst = TRUE, pred_bands = .95)
